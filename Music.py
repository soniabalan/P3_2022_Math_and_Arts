...
def get_song_from_seq(number_sequence, c_flag):
    '''
    ...
    if c_flag == 0: #get single note melody   
        for nr in number_sequence:
            if nr.isdigit() == True:
                music_notes+= '-'+ key_notes[nr]
        song = get_song_data(music_notes)

    else: #get chord melody
        for nr in number_sequence:
            if nr.isdigit() == True:
                 music_notes+='-' + key_chords[nr]
        song = get_chord_data(music_notes[1:])
...
def main():
    ...
    for x in Sequences:
        data = get_song_from_seq(Sequences[x], 0)
        write(('music_'+str(x)+'_20_01.wav'), samplerate, data.astype(np.int16))
        datac = get_song_from_seq(Sequences[x], 1)
        write(('music_'+str(x)+'_20_01_chords.wav'), samplerate, datac.astype(np.int16))
